...
// 生圖候選模型（依序嘗試；不同帳號/方案可用者不同）
const IMAGE_MODELS = ['gemini-2.5-flash-image', 'gemini-3.1-flash-image', 'gemini-2.0-flash-preview-image-generation']

// Gemini 生圖（用使用者免費金鑰）；成功回 data URL，全部失敗丟 AIError（附原因供畫面顯示）
export async function geminiImage(key: string, prompt: string): Promise<string> {
  if (!key) throw new AIError('尚未設定 Gemini 金鑰')
  let lastErr = '未知錯誤'
  for (const m of IMAGE_MODELS) {
    try {
      const res = await fetch(`${BASE}/${m}:generateContent?key=${encodeURIComponent(key)}`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ contents: [{ role: 'user', parts: [{ text: `Clean flat child-friendly educational illustration, no text in image: ${prompt}` }] }], generationConfig: { responseModalities: ['TEXT', 'IMAGE'] } }),
      })
      if (!res.ok) {
        let detail = ''
        try { const j = await res.json(); detail = j?.error?.message ?? '' } catch { /* ignore */ }
        lastErr = `${res.status}${detail ? '：' + detail.slice(0, 120) : ''}`
        continue
      }
  ...
  throw new AIError(`生圖失敗（${lastErr}）。你的免費金鑰可能未開通圖片生成或額度不足。`)
}
